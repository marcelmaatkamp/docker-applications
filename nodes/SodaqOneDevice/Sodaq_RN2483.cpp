...
// Gets the Down counter from the module.
uint32_t Sodaq_RN2483::getDownCntr()
{
  String output;
  debugPrintLn("[getDownCntr]");

  this->loraStream->print(STR_CMD_GET_DNCTR);
  this->loraStream->print(CRLF);
  ...
  while (millis() < start + DEFAULT_TIMEOUT) {
    ...
    if (size > 0) {
      ...
      {
        output += (char)this->inputBuffer[i];
      }
      debugPrintLn(output);
      return (uint32_t)(output.toInt());
    }
  }

  debugPrint("[getDownCntr] Timed out without a response!");
  return -1;
}

// Gets the Up counter from the module.
uint32_t Sodaq_RN2483::getUpCntr()
{
  String output;
  debugPrintLn("[getUpCntr]");

  this->loraStream->print(STR_CMD_GET_UPCTR);
  this->loraStream->print(CRLF);
  ...
  debugPrint("[getUpCntr] Timed out without a response!");
  return -1;
}
...
bool Sodaq_RN2483::setUpCntr(uint32_t cntr)
{
  String input = String(cntr);
  
  debugPrint("[set uplink counter]");
  debugPrintLn(input);
  
  setMacParam(STR_CMD_SET_UPCTR, input.c_str());
  save();
  
  return true;
}

bool Sodaq_RN2483::setDownCntr(uint32_t cntr)
{
  String input = String(cntr);
  
  debugPrintLn("[set downlink counter]");
  
  setMacParam(STR_CMD_SET_DNCTR, input.c_str());
  save();
